...
Before geographic aggregation, detections are summarized at the individual substation-image level. For each image, the four retained class counts are reported separately and summed into an `Overall` count. The distribution figure reports box-and-whisker summaries with individual image observations, class means, and overall means by planning region. Detections are then aggregated to characterize geographic patterns at state and planning-region scales. Figure-generation scripts are in `graphs/`. They use repository-relative paths so they can be run from a cloned checkout without editing local file locations.
...
- `substation_component_distribution_summary.csv` — generated summary statistics for the substation-level distribution analysis

Figures generated by the reproduction command are also written to this directory, including `substation_component_distributions.png`. In that figure, `Overall` means the per-image sum of Transformer, Reactor, Circuit Breaker, and Alt Energy detections; it does not include Control or Power Lines. Reported values are model-detected components per substation image rather than exhaustive physical equipment inventories.
...
The tests check core repository invariants, including agreement between image metadata and prediction identifiers, consistency of FERC totals with the retained component classes, and the substation-level overall-count calculation used in the distribution analysis.
